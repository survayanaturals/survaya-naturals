...
function doPost(e) {
  try {
    ...
    sheet.appendRow([
      ...
      data.total,                       // J: Total
      'Pending Verification',           // K: PaymentStatus
...
function doGet(e) {
  try {
    ...
    if (!orderId) {
      return ContentService
        .createTextOutput(JSON.stringify({ success: false, error: 'No orderId provided' }))
        .setMimeType(ContentService.MimeType.JSON)
    }
    ...
    for (let i = 1; i < data.length; i++) {
      if (data[i][0] === orderId) {
        ...
        return ContentService
          .createTextOutput(JSON.stringify({ success: true, data: order }))
          .setMimeType(ContentService.MimeType.JSON)
      }
    }
